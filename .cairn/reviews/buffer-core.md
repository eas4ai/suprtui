...
findings:
  ...
  - open: the precision claim above does not reproduce against the current tree. The isolation was restored to the render-core activation content per LOOP-035 (scope answer loop-035, `instead`), so the counting test re-owns the clear assertion and the two faults couple again. Remove this note when the isolation is re-applied as declared work.
